...
module.exports = function Twitch(app, mongo) {
  ...
  twitch.on('resub', (channel, username, months, message) => {
  ...
  twitch.on('cheer', (channel, userstate, message) => {
    ...
    logger.ingo(`${username} has cheered with ${userstate.bits} bit(s) [${message}]`);
    const data = {
      username,
      bits: userstate.bits,
      message,
  ...
  twitch.on('hosted', (channel, username, viewers) => {
    logger.info(`${username} hosted the channel: +${viewers} viewer(s)`);
