...
module.exports = function Twitch(app, mongo) {
  ...
  twitch.on('cheer', (channel, userstate, message) => {
    ...
    const username = getNonLocalizedName(userstate.username, userstate['display-name']);
  ...
  twitch.on('chat', (channel, userstate, message) => {
    ...
    const oddshot = message.match(/https:\/\/oddshot.tv\/shot\/\w*/);
    const twitchClip = message.match(/https:\/\/clips.twitch.tv\/\w*\/\w*/);
  ...
  function getDisplayName(username, callback) {
    ...
    request.get({
      ...
      if (!err && res.statusCode === 200) {
        return callback(getNonLocalizedName(body.name, body.display_name));
      } else {
  ...
  function getNonLocalizedName(name, displayName) {
    if (displayName) {
