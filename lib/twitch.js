...
module.exports = function Twitch(app, mongo) {
  ...
  const oddshotReg = /https:\/\/oddshot\.tv\/s\/\S*/;
  const twitchClipsReg = /https:\/\/clips\.twitch\.tv\/\S*/;
  ...
  twitch.on('chat', (channel, userstate, message) => {
    ...
    let uri;
    if (oddshotReg.test(message)) {
      uri = message.match(oddshotReg)[0];
      logger.info(`Oddshot: ${uri}`);
    } else if (twitchClipsReg.test(message)) {
      uri = message.match(twitchClipsReg)[0];
      logger.info(`TwitchClip: ${uri}`);
    } else {
      return;
    }
    mongo.clips.findOne({ uri })
      .then(result => {
        logger.debug(Promise.resolve(result));
        if (result) return;
        result = mongo.clips({ uri });
        result.save().catch(logger.error);
        if (oddshotReg.test(message)) myEvents.emit('clip', { userstate, message, link: uri });
        if (twitchClipsReg.test(message)) myEvents.emit('clip', { userstate, message, link: uri });
      }).catch(logger.error);
  });
