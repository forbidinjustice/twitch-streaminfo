...
module.exports = function (options, app, mongo) {
    ...
    function getDisplayName(username, callback) {
        request.get({
            url: encodeURI("https://api.twitch.tv/kraken/channels/" + username.toLowerCase() + "?client_id=" + options.twitch.client_id),
            json: true
        }, (err, res, body) => {
    ...
    app.io.on('connection', (socket) => {
        ...
        socket.on('get_bit_badges', () => {
            request.get({
                url: encodeURI("https://badges.twitch.tv/v1/badges/global/display?client_id=" + options.twitch.client_id),
                json: true
            }, (err, res, body) => {
