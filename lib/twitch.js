...
module.exports = function (options, app, mongo) {
    ...
    twitch.on("roomstate", (channel, roomstate) => {
        ...
        if (roomstate.r9k != null) options.twitch.status.r9k = roomstate.r9k;
    ...
    twitch.on("emoteonly", function (channel, enabled) {
        ...
        options.twitch.status['emote-only'] = enabled;
        app.io.emit('twitch_status', {'emote-only': enabled});
    });

    //Channel toggled R9K Mode
    twitch.on("r9kbeta", function (channel, enabled) {
        utils.log("r9k in " + channel + ": " + enabled);
        options.twitch.status.r9k = enabled;
        app.io.emit('twitch_status', {r9k: enabled});
    });
    ...
    twitch.on("subscribers", function (channel, enabled) {
        ...
        options.twitch.status['sub-mode'] = enabled;
        app.io.emit('twitch_status', {'sub-mode': enabled});
    });
