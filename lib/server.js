...
module.exports = function (options, mongo) {
    ...
    io.on('connection', (socket) => {
        ...
        mongo.subscribers.find({}, (err, results) => {
            if (!err) socket.emit('subscribers_list', results.reverse()); //Send list of recent subscribers
        });

        mongo.cheers.find({}, (err, results) => {
            if (!err) socket.emit('cheers_list', results.reverse()); //Send list of recent cheers
        });
