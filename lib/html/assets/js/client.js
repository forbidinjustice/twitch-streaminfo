...
io.on('queues_list', (data) => {
    ...
    $('.queues ul').empty();
    if (data.length == 0) {
        $('.queues > ul').append("<li><div class='time' style='visibility: hidden'>Date</div><span class='username'>Queues</span></li>");
        return;
    }
    for (let i = 0; i < data.length; i++) {
        $('.queues > ul').append("<li><div class='time' style='visibility: hidden'>Date</div><span class='username'>" +
