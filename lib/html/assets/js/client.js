...
$(document).ready(function () {
    $('.channel_controls input').click(function (e) {
        e.preventDefault();
        let command = "/" + $(this).parent().parent()[0].className + ($(this).prop('checked') ? "" : "off");
...
function addSub(data) {
    ...
    $('.columns .subs ul').prepend("<li>" + data.username + " <span class='amount'>" + months +
...
function addCheer(data) {
    ...
    $('.columns .cheers ul').prepend("<li>" + data.username + " <span class='amount'>(" + data.bits +
        ")</span><div class='message'>" + data.message + "</div></li>");
