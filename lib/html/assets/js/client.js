...
function addSub(data) {
    ...
    let time = "10:50pm 7/2/16";
    let months = data.months > 1 ? "(" + data.months + " months)" : "(NEW SUB)";
    let message = data.message ? data.message : "";
    $('.columns .subs ul').prepend("<li><div class='time'>" + time + "</div>" + data.username + " <span class='amount'>" + months +
        "</span><div class='message'>" + message + "</div></li>");
...
function addCheer(data) {
    ...
    $('.columns .cheers ul').prepend("<li><div class='time'>" + time + "</div>" + data.username + " <span class='amount tie_" +
        getBitTier(data.bits) + "'>(" + data.bits + ")</span><div class='message'>" + message + "</div></li>");
