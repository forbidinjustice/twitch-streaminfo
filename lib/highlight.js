...
function recordHighlight(query, callback) {
  const hostUri = `http://tmi.twitch.tv/hosts?include_logins=1&host=51533859`;
  logger.debug(hostUri);
  fetch(hostUri)
    .then(hostBody => {
      logger.debug();
      const channel = hostBody.hosts[0].target_login || config.twitch.channel.replace('#', '').toLowerCase();
      const streamUri = `https://api.twitch.tv/kraken/streams/${channel}?client_id=${config.twitch.client_id}`;
      ...
      fetch(streamUri)
        .then(body => {
          ...
          const vUri = `https://api.twitch.tv/kraken/channels/${channel}` +
          ...
          function addToSheet(pbURL) {
            my_sheet.useServiceAccountAuth(my_sheet_creds, error => {
              ...
              my_sheet.addRow(1, {
                ...
                const hosted = hostBody.hosts[0].target_login ? 'hosted ' : '';
